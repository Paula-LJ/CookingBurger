...
public class Spawn : MonoBehaviour
{
    ...
    public void OrderBurger()
    {
		...
		{
			if (y == 1)
				NewI = Instantiate(IngredientAlways[y], NewSpeak.transform.position + new Vector3(4f - y, 6, 0), IngredientAlways[y].transform.rotation);
			else
				NewI = Instantiate(IngredientAlways[y], NewSpeak.transform.position + new Vector3(5 - (y * 3), 6, 0), IngredientAlways[y].transform.rotation);
			NewI.transform.parent = FolderClient.transform;

		}

		ranPrefab = Random.Range(0, IngredientUncooked.Length);
        NewI = Instantiate(IngredientUncooked[ranPrefab], NewSpeak.transform.position + new Vector3(-4, 6, 0) , IngredientUncooked[ranPrefab].transform.rotation);
        NewI.transform.parent = FolderClient.transform;

        Debug.Log(IngredientUncooked[ranPrefab]);
